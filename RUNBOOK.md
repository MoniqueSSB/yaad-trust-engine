...
**Check a ContentSid secret actually holds a ContentSid.** `npx supabase secrets list --project-ref leffyisvfvjwzilydlwf` prints a SHA-256 of each value rather than the value, which is still enough to catch the two mistakes that matter. **Two different templates must never show the same digest**: one ContentSid cannot belong to two templates, so a match means a placeholder or a copy-paste went in. And a digest can be tested against a suspected string directly, `printf '%s' 'HXpasteithere' | shasum -a 256`, without ever printing the secret.

This is not hypothetical. On 4 September 2026 both `TWILIO_CONTENT_SID_DAILY_CHECKIN` and `TWILIO_CONTENT_SID_APPROVAL` were set to the literal placeholder `HXpasteithere`, straight out of a copied command, and matching digests are what found it. Both were unset the same evening. The damage would not have shown up for months: `yaad-daily-checkin` stops reporting "not set" the moment the secret exists, tries to send against an invalid SID, and writes `daily_checkin_log` regardless of whether the send worked, so every worker reads as asked and none of them were. **Unset is the correct state until Meta has actually approved the template.** Both functions are written for it.
...
**It needs the Supabase CLI authenticated**, the same auth `supabase functions list` uses. If you have not logged in on this machine it stops with the CLI's own words, `Access token not provided`, and the fix is `npx supabase login` once, or exporting `SUPABASE_ACCESS_TOKEN`. It prints whatever the CLI actually said rather than guessing: the first version of this script hid stderr and guessed, and produced a Python traceback for a missing login and a confident wrong answer for a malformed project ref.
