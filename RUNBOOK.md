...
8. **The worker says no WhatsApp arrived.** It is free text, so it only arrives if they have messaged the Yaadly number in the last 24 hours. Check Supabase, Edge Functions, `yaad-notify-client`, Logs, for kind `materials_sent_worker`, and tell them by phone. Their portal shows the release as Sent either way.
9. **Proving the sent rules hold:** run `supabase/tests/materials_sent_guards.sql` with `execute_sql`. Eleven lines, all PASS. Nothing is kept, and the WhatsApp it queues is thrown away with everything else.
10. **Deploy order for this change:** apply `20260914190000` first, then deploy `yaad-notify-client` (it keeps `--no-verify-jwt`, it is on the CLAUDE.md §12 list), then the web app, then the desk. The portal and the desk read `sent_at`; before the migration that read fails and the release list shows nothing.
