...
**Not enabled yet (Stripe support, 16 Sep 2026).** Global Payouts is switched on from the Dashboard, not by support: Global Payouts in the left navigation, **Get started**, accept the Global Payouts Terms of Service, complete any missing business detail. Usually minutes; occasionally 2 to 3 weeks of Enhanced Due Diligence. Until that is done every recipient call fails with "not enabled" (step 4 below). Jamaica is on Stripe's supported table for a UK sender: individual, JMD, local bank, email and name. The Terms of Service are the licensing note the decisions log says a solicitor should see first.
