...
**A `draft` invoice needs a person to click Send before it reaches anyone.** The desk's Job Invoices view shows a **Send** button on any `draft` row (added alongside the auto-raise trigger, since before it there was no invoice that could exist in `draft` state without someone having just clicked "Raise & send" in the same breath). This is deliberate: raising is automatic, emailing a real client is still a human's click, matching how every other invoice in this codebase works. If a worker-pay invoice is sitting unsent for a while, that is either nobody having looked, or a genuine backlog worth asking about, not a sign anything is broken.
