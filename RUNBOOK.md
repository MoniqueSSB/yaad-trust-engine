...
**The shared secret lives only as a hash.** `app_settings.notify_trigger_secret_sha256` stores the SHA-256, never the plaintext. The plaintext is baked into four trigger function bodies, not three: `notify_client_quote_arrived`, `notify_client_on_job_change`, `notify_client_dispute_raised` (all three from `20260831i`), and `notify_client_worker_arrived` (`20260831m`, added after this section was first written). A mismatch between what a trigger sends and what the hash expects fails closed; `yaad-notify-client` returns 401 rather than notifying on a bad secret.

**A fifth copy lives outside Postgres entirely.** `yaad-job-health` holds the same plaintext as the Edge Function secret `YAAD_CRON_SECRET`, to authenticate its own calls into `yaad-notify-client` for `job_delayed`. Rotating the database side without updating this breaks `job_delayed` silently: `yaad-notify-client` will 401 it and nothing else will say so.

**Rotated 31 Aug 2026** (`20260831x`), after the plaintext appeared in a Claude Code session's own tool output: `pg_get_functiondef` against `notify_client_on_job_change`, run to read its trigger logic, returns the whole function body, literal included. That migration is also the template for next time: it regenerates the secret with `gen_random_bytes` entirely inside Postgres, rewrites all four function bodies and the hash in one transaction, and never selects the plaintext back out to whoever is running it. The one exception is deliberate: it also calls `vault.create_secret()`, so the new plaintext leaves the database exactly once, encrypted, for a human to carry across by hand. **To finish a rotation:** open Supabase Studio's own SQL editor (a session the migration has no part in) and run `select decrypted_secret from vault.decrypted_secrets where name = 'notify_trigger_secret_plaintext_20260831x'` (or whatever name the latest rotation used), copy the value, run `supabase secrets set YAAD_CRON_SECRET='<value>' --project-ref leffyisvfvjwzilydlwf` yourself, then delete the Vault row (`select vault.delete_secret(...)` by id, from that same table) so it does not sit there as a sixth copy.
