...
Two secrets on this project have been exposed and rotated: an API key, and the notify trigger secret, committed as a literal in two migrations on 1 and 2 Sep 2026 and rotated into Vault on 3 Sep. The CI secrets job is a cheap backstop, not a substitute for care; it has a step for quoted 64 character hex strings because of the second one.
...
**Rotating it, now that it is in Vault.** Update the Vault secret (`select vault.update_secret(id, encode(extensions.gen_random_bytes(32),'hex')) from vault.secrets where name = 'notify_trigger_secret';`), then set `app_settings.notify_trigger_secret_sha256` to the SHA-256 of the new value. Nothing else. No function body changes, and there is no environment copy to keep in step: `yaad-job-health`, `yaad-followup-check` and `yaad-evidence-landed-check` read the same Vault value over RPC with `public.notify_trigger_secret()`, cache it per isolate, and drop the cache when the hub refuses them (it answers 403, not 401) so they pick a rotation up by themselves. **Do not set `YAAD_CRON_SECRET` to this value.** Since 3 Sep 2026 it is only those functions' own inbound admin secret, deliberately unrelated. If you deploy one of those three and it sends `YAAD_CRON_SECRET` to the hub, you have deployed a copy from before the Vault lookup, and every call it makes is refused: that happened from 5 to 13 Sep 2026 (DECISIONS.md, 13 Sep). Check with `grep -n notifySecret supabase/functions/<name>/index.ts` before deploying.
...
**If a timer fires (`fired_at` set) but nobody was actually told**, check `yaad-evidence-landed-check`'s own logs for a `403` from `yaad-notify-client` first: the function reads the secret from Vault over RPC (see "Rotating it, now that it is in Vault", above), so a refusal there means either the RPC failed, which it logs as `notify_trigger_secret rpc failed`, or a copy from before the Vault lookup was deployed, which sends `YAAD_CRON_SECRET` instead. If that check comes back clean, the fault is downstream in `yaad-notify-client` itself (no worker phone on file, an AI review failure, a Twilio send failure), the same causes as `evidence_landed`'s existing failure modes elsewhere in this file, not anything specific to the debounce.
...
- A row at `'ready'` but still no pack: check its `guardrail` column. Any of `price_language_detected`, `banned_language_detected` or `foreign_text_detected` being `true` holds it back deliberately. It shows on the desk's Kickoff packs view, under "Drafts that did not become a pack", with the flags that held it; fix or redraft it. A clean draft written for no quote or booking (the kickoff wizard's own) shows there too, with its own "Link to a job" and "Link to a service" buttons. The Kickoff Drafts view that used to hold all of these was taken out of the menu on 14 Sep 2026.
...
**Since 17 September 2026 nothing is filed until the worker replies 1.** With one active job the words are held in an `update_draft` session and read back; a reply of `1` files them, `no` drops them, any other words replace the draft, and photos sent next take the waiting words as their caption. A lone character or number is never an update. Outcomes on the trace: `held_for_confirm`, `filed`, `draft_kept`, `draft_dropped`, `draft_job_not_open`, `too_short_not_update`. If a worker says their update never reached the client, check whether they replied 1: an unconfirmed draft is dropped with the session after 48 hours. The rule lives in `supabase/functions/yaad-inbound/update-draft.ts`, tested in `update-draft_test.ts`. The paragraph above describing "Got it, on record" as the first reply is now the reply to the 1, not to the update.

## A worker says a WhatsApp message from Yaadly never arrived

**WhatsApp only delivers ordinary text within 24 hours of the worker's own last message.** Outside that, Twilio accepts the send and it fails later as `63016`, which shows on the delivery page but never reaches `yaad-notify-client`. Since 17 September 2026 the function reads `wa_inbound_seen` first: a worker with nothing in from their number for 23 hours gets the approved template `yaadly_worker_job_update_v1` (a one-line summary per kind and the job link, `yaad-notify-client/worker-template.ts`). The drafted report (`evidence_landed`) is deliberately never sent as a template, because the worker has to read it before replying 1.

1. **Is the template switched on?** `select value from app_settings where key = 'twilio_content_sid_worker_update';` Empty means not yet approved by Meta, and workers outside the window still get free text that fails.
2. **Submit it** (once): call `yaad-twilio-setup` with `{"action":"create-worker-update"}`, gated like the scheduled functions. It creates the template, submits it to Meta as UTILITY and stores the id as `twilio_content_sid_worker_update_pending`.
3. **Check approval**: `{"action":"worker-update-status"}`. When Meta says approved it writes `twilio_content_sid_worker_update` and the template is live on the next message. If rejected, the reason comes back; change the words in `yaad-twilio-setup/content.ts` and `worker-template.ts` together (a test holds them equal), bump the `_v1` name, and submit again.
4. **The trace** carries `yaadly.notify.worker_template` true or false for every worker message.
...
**A fourth sweep, added 9 September 2026: no page may say a client pays a tradesperson directly.** From 4 to 9 September the site carried an "oversight only" lane: keep your own contractor, pay them directly, Yaadly holds none of the money for the building work, Yaadly sells the eyes and invoices its own fee separately. Monique said it is untrue and had it taken off every page, the client portal, the WhatsApp reply facts in `supabase/functions/yaad-inbound/faq.ts`, and `docs/COPY-GUIDELINES.md`. The client pays Yaadly on every job and every service, and Yaadly engages and pays the tradesperson.

```bash
grep -rniE "your contractor stays|pay (him|them|your contractor) directly|paying them directly|holds none of|sells the eyes|supplies the eyes|own fee is invoiced separately|oversight only|own contractor" docs/*.html web/app web/components supabase/functions/yaad-inbound/faq.ts
```

Expected: no hits. `preview/index.html` is the illustrative prototype and still carries the old wording; it is not swept here because it is not the offer, but it is public at `yaadly.co.uk/preview/`, so if anyone links to it the same edit is due there. The Deposit Protection Check card on `services.html` says "Your contractor and their quote, checked before you pay anybody"; that is a description of the check, not a payment claim, and stays.

After changing `faq.ts`, redeploy `yaad-inbound` from disk (RUNBOOK deploy rules apply, read the live `verify_jwt` first), or the WhatsApp assistant keeps describing the removed lane.
...
3. Worker-facing screens: `web/components/QuotePanel.tsx`, `web/components/portal/FeeBreakdown.tsx`, `web/components/portal/MoneyPanel.tsx`, `web/components/portal/JobRail.tsx`, `web/app/portal/(gated)/jobs/[id]/page.tsx`, `web/app/portal/(gated)/worker/page.tsx`, `web/app/trades/page.tsx`. Grep for `0.95`, `5%` and `15%` rather than trusting this list. `JobRail.tsx` is the one to remember: the figure reaches it from the job page, but the label is its own text, and until 14 September 2026 that label said "15%" on the worker's side while the page handed it the client's fee, so the rail read three times the real deduction for five days after the cut. The lines under the rail's headline must sum to the headline on both sides; if they do not, the rail and `MoneyPanel` are being fed different inputs.
...
4. **There is no full rehearsal, only a partial one (tried 19 to 20 Sep 2026).** The account's legacy "Test mode" key refuses Global Payouts outright ("Please switch to an account created in a sandbox"). The **Yaadly LTD** sandbox (`acct_1U43aOKRGtW3lkJK`) accepts recipients, and Stripe's Jamaican test account (routing 111, branch 00000, account 000123456789) takes a worker all the way to **ready**, but that sandbox has **no financial account**, the list comes back empty, and `POST /v2/money_management/financial_accounts` answers 404, so nothing can be sent from it. Stripe support would have to switch payouts on for that sandbox. Until then the first real payout is the proof: small, live, to a real worker. If you run the partial rehearsal, the live key and the payouts switch are shared with card payments, so while the sandbox key is set the portal's Pay by card and the emailed pay link take nothing. Put the live key back the same evening, and clear any sandbox `stripe_recipient_id` from `worker_profiles` afterwards, because a live key cannot read a sandbox recipient.
...
**Setting a Stripe key without mangling it.** Three attempts on 19 to 20 Sep went in wrong: the placeholder word typed literally twice, then a truncated paste ("malformed API Key" in the `yaad-payout-setup` logs). What worked: copy the key in Stripe, then `supabase secrets set --project-ref leffyisvfvjwzilydlwf STRIPE_SECRET_KEY="$(pbpaste | tr -d '[:space:]')"`. Check it took by pressing Set up with Stripe on the worker page and reading the function logs; a live key opens Stripe's real form on the live account.

## A worker cannot be paid because the client has not paid (20260917180000, 20260919160000)
...
3. **A row says the check could not be read.** The migration is not applied yet, or the session expired. Sign in again; if it persists, check `20260917180000` and `20260919160000` are in `schema_migrations`.
...
5. **Proving it holds:** run `supabase/tests/worker_pay_client_paid_guards.sql` with `execute_sql`. Ten lines, all PASS. It relies on the TEST jobs JOB-TEST-KICKOFF-1, JOB-TEST-WAPAY-3 and JOB-TEST-WA-CONFIRM as they stood on 19 Sep 2026; if those change, a FAIL there means the data moved, so read the reason before touching the function. Lines 3, 4 and 7 had their expected wording changed by `20260919160000` and each still asserts a refusal; lines 9 and 10 are new and prove the two halves of the narrowed rule, the fallback allowing and the stage rule still biting where the stage is genuinely billed.
...
7. **Deploy order:** apply `20260914230000` first, then deploy `yaad-notify-client` (it keeps `--no-verify-jwt`, it is on the CLAUDE.md §12 list), then the web app, then the desk. The portal and the desk read `paid_method` and `paid_reference`; before the migration that read fails and the invoice lists come back empty.
...
## Workers give bank details straight to Wise, and a call-back comes first (20260914240000)

1. **Switching it on, in this order.** Apply `20260914240000`. In Wise, create a personal API token for the Yaadly Ltd business account, with full access (a read-only token cannot create recipients; a UK token can never fund a transfer). In your own terminal, never in chat: `supabase secrets set WISE_API_TOKEN=PASTE_IT_HERE --project-ref leffyisvfvjwzilydlwf`. Then from `main`: `supabase/functions/sync-shared.sh`, `supabase functions deploy yaad-wise-recipient --project-ref leffyisvfvjwzilydlwf` (platform JWT check stays ON, no flag), `supabase functions deploy yaad-notify-client --project-ref leffyisvfvjwzilydlwf --no-verify-jwt`, then the web app, then the desk.
2. **Check it is behind the login:** `curl -s -o /dev/null -w "%{http_code}\n" -X POST https://leffyisvfvjwzilydlwf.supabase.co/functions/v1/yaad-wise-recipient` must answer 401.
3. **Try it:** sign in as the test worker, How Yaadly pays you, fill the form with a real-looking test entry, send. It should appear in Wise, Recipients, and on the desk under Pay workers, Bank details to check. Delete the test recipient in Wise afterwards.
4. **Before paying anyone:** phone them on the number on file, check the details against the recipient in Wise (bank, branch, last four digits, name matches their ID), press **Call-back done**. Until then Mark as sent is refused, on Pay workers and on Materials tranches.
5. **A worker changed their details:** a new recipient appears in Wise and their call-back is cleared automatically. Call again, then delete the old recipient in Wise.
...
**What is still true:** Twilio's documentation does say a Messaging Service is a prerequisite for Content Templates, so naming it is correct and it stays. **What is not true is that naming it is what fixed this.** The one account-side change inside that window was Meta approving `yaadly_worker_job_update_v1`, the first template ever approved on this WhatsApp sender, which is a plausible unlock and is not proof either. **Do not write the cause down as settled.** If the menu ever stops again, test the probe with and without `MessagingServiceSid` before changing any code.

**Found, 19 Sep 2026, and believed for about twenty minutes: the send named no Messaging Service.** Twilio's own documentation: "A Messaging Service is a prerequisite for using Content Templates." `sendPhaseMenu` was passing a `From` number and a `ContentSid` and nothing else, which Twilio refuses with **20422 Invalid Parameter**, every time, synchronously. A plain `Body` send needs no Messaging Service, which is why every other message from the same number went out normally and this looked like a template fault for four days. Two false leads on the way, both recorded here so nobody spends the afternoon again: the template itself is correct in every field, and a list picker can never be approved, so approval was never the missing piece.

**It was never only the menu. Five functions had it (19 Sep 2026).** `yaad-inbound`, `yaad-notify-client`, `yaad-enquiry`, `yaad-daily-checkin` and `yaad-desk-reply` all sent a `ContentSid` with no Messaging Service, so **every** WhatsApp template Yaadly sends was being refused, not just the section menu. That is why the "stage released" and "materials sent" notices were not arriving either. The rule now lives in one place, `withMessagingService()` in `_shared/twilio-status.ts`, called at every send site, and a test in `_shared/twilio-status_test.ts` reads the functions themselves and fails if any `ContentSid:` send does not name a service. It caught one of the five that had been missed while it was being written, which is the argument for the test.

**Set it once:** `TWILIO_MESSAGING_SERVICE_SID` is a function secret (set 19 Sep to `MG9226…`), read by every function through the shared helper. `yaad-inbound` also accepts the `twilio_messaging_service_sid` row in `app_settings` so the desk can blank it.

**The fix:** the send now carries `MessagingServiceSid` as well as `From`. **The number does not have to be in the service's sender pool** (Twilio: "you do not have to add the channel sender to the messaging service's sender pool, but a valid messaging service is still required"), and Yaadly's one service, `Default Messaging Service for Conversations` (`MG9226…`), has an empty pool and is used exactly that way. It is read from the `TWILIO_MESSAGING_SERVICE_SID` secret, else the `twilio_messaging_service_sid` row in `app_settings`, the same shape as the template id. Record it with `{"action":"use-messaging-service","messagingServiceSid":"MG…"}`, which checks it against Twilio before writing so a typo cannot quietly turn the menu off. **Either row blank means no menu and the typed letters instead**, and with no Messaging Service set the function does not even attempt the send, it logs and falls back, rather than failing on the wire.

**To force a refusal without waiting for a worker:** `{"action":"test-send-section-menu","to":"4477..."}` against `yaad-twilio-setup` sends the menu once to the number you type in, and hands back Twilio's whole error body, code and all. **It sends a real WhatsApp message**, the number is never defaulted or looked up, and the send is recorded on the desk's "Did it arrive" page as kind `section menu test` like any other. Nothing is filed and no job is touched.

**To read the template back:** `{"action":"describe-section-menu"}` against `yaad-twilio-setup`, presented the same way the scheduled jobs present themselves (the cron secret in the body) or from an admin session. It returns the recorded ContentSid, Twilio's live copy of the template, its WhatsApp approval status, and what `content.ts` would create, so the two can be compared side by side. It creates nothing, sends nothing and writes no setting.
...
6. **Read it on the desk first.** Click the row and the report opens as a page: verdict, every finding with its rating, the payment stages, the message, the checklist, scope, referrals, what was scrubbed, and the transcript under a fold. The actions sit at the foot of that page. Teal is done, mango is yours to do, coral is a gap in the paperwork.
7. **Open the document.** Builds the full report in a new tab: cover with the logo, header on every page, verdict, red flags, payment schedule, message to the builder, checklist, scope in and out, the variation rule, referrals. Press **Print** in the top right and choose **Save as PDF**. Before issue it opens with the blanks showing, which is useful for reading it whole before you rate.
