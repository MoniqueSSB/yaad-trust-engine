-- A photograph says whether it is a before or an after.
--
...
-- photograph is one.
--
-- NULL IS AN HONEST THIRD ANSWER. It means nobody said, and it must not read
-- as "no". Every row filed before today is null, and a worker who does not
-- answer the question still gets their evidence filed. Nothing about this
-- column blocks anything: it records, it reports, and a named human still
-- decides. Whether a missing before ever becomes a gate on approval is a
-- separate decision and is the founder's, not this migration's.
...
-- stage of the work, so a before or an after on it means nothing and is
-- refused rather than quietly stored and later counted.
...
alter table public.evidence add constraint evidence_phase_chk
  check (
    phase is null
    or (phase in ('before', 'after') and kind is distinct from 'materials')
  );

comment on column public.evidence.phase is
  'before or after, declared by the person filing it in answer to a direct question, never read out of the label. Null means nobody said, which is not the same as no, and is what every row filed before 5 Sep 2026 carries. Refused on kind = materials, where the distinction has no meaning.';

-- The stage where a client is most likely to want the pairing side by side.
...
--
-- Three new columns on the end, so CREATE OR REPLACE keeps the existing shape
