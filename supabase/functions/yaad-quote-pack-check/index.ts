...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let allowed = false;
    if (presented) {
      const { data: st, error: stErr } = await admin.from("app_settings").select("value").eq("key", "quote_pack_check_cron_secret_sha256").maybeSingle();
      ...
      if (stErr) {
        console.error(`yaad-quote-pack-check: could not read the cron secret hash: ${stErr.message}`);
