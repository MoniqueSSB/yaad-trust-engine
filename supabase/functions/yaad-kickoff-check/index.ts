...
//
// 1. A QUOTE the client has asked to see a Kickoff Pack for
//    (job_quotes.status = 'kickoff_requested'), with no draft yet, gets one
//    requested, from yaad-kickoff, authenticated as the service role - not
//    a shared secret baked into a trigger the way the notify triggers work,
//    and not a weaker check inside yaad-kickoff either. The service role
//    key is already the single most-trusted secret in this system and
//    already never reaches a browser (CLAUDE.md §6); reusing it here adds
//    no new secret to leak or drift, unlike minting another shared one.
//    Per-quote, not per-job, since 1 Sep 2026: a client can accept more
//    than one quote and compare Kickoff Packs before choosing, so a job
//    can have more than one draft in flight, one per worker's price.
//
...
//    manual door.
//
// choose_worker() now requires the CHOSEN quote's own Kickoff Pack to be
// confirmed by both sides before it will book anyone
// (20260901f_client_can_accept_more_than_one_quote_before_choosing): this
// function is what gets that pack drafted and issued in the first place,
// asynchronously, within one poll interval of the client accepting.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Phase 1: request a draft for every quote the client has asked to
    // see a Kickoff Pack for, that doesn't have one drafting or ready
    // already, and hasn't already failed three times running. Founder's
    // own correction, 1 Sep 2026: a client can accept more than one quote,
    // so this now works per QUOTE, not per job - a job with two accepted
    // quotes gets two drafts in flight, one against each worker's price. ──
    const { data: reqQuotes } = await admin.from("job_quotes")
      .select("id,job_id").eq("status", "kickoff_requested") as { data: { id: string; job_id: string }[] | null };

    const jobIds = [...new Set((reqQuotes ?? []).map((q) => q.job_id))];
    const { data: jobRows } = jobIds.length
      ? await admin.from("jobs")
          .select("id,title,client_name,parish,descr,trade,urgency,access_type,access_contact")
          .in("id", jobIds) as { data: Job[] | null }
      : { data: [] as Job[] };
    const jobsById = new Map((jobRows ?? []).map((j) => [j.id, j]));

    const { data: draftRows } = await admin.from("kickoff_drafts")
      .select("quote_id,status").not("quote_id", "is", null);
    const { data: packRows } = await admin.from("kickoff_packs").select("quote_id").not("quote_id", "is", null);

    const hasActiveOrReadyDraft = new Set((draftRows ?? []).filter((d) => d.status === "drafting" || d.status === "ready").map((d) => d.quote_id));
    const hasPack = new Set((packRows ?? []).map((p) => p.quote_id));
    const failedCounts = new Map<string, number>();
    for (const d of draftRows ?? []) {
      if (d.status === "failed") failedCounts.set(d.quote_id, (failedCounts.get(d.quote_id) ?? 0) + 1);
    }

    let requested = 0, skippedNoBrief = 0, skippedTooManyFailures = 0, requestFailed = 0;
    const requestErrors: string[] = [];
    for (const q of reqQuotes ?? []) {
      if (hasActiveOrReadyDraft.has(q.id) || hasPack.has(q.id)) continue;
      if ((failedCounts.get(q.id) ?? 0) >= 3) { skippedTooManyFailures++; continue; }
      const j = jobsById.get(q.job_id);
      if (!j) continue;
      const intake = jobToIntake(j);
      if (!intake.brief) { skippedNoBrief++; continue; }

      const errMsg = await trace.span("yaad-kickoff request", SpanKind.INTERNAL, { "yaadly.kickoff_check.job_id": j.id, "yaadly.kickoff_check.quote_id": q.id }, async (s) => {
        ...
        const r = await fetch(`${SUPABASE_URL}/functions/v1/yaad-kickoff`, {
          ...
          body: JSON.stringify({ jobId: j.id, quoteId: q.id, intake }),
          signal: AbortSignal.timeout(15000),
        });
        s.setAttributes({ "http.response.status_code": r.status });
        if (r.ok) return "";
        const t = await r.text().catch(() => "");
        return `${j.id}/${q.id}: ${r.status} ${t.slice(0, 200)}`;
      });
    ...
    const { data: readyDrafts } = await admin.from("kickoff_drafts")
      ...
      .not("quote_id", "is", null).eq("status", "ready");

    let linked = 0, heldForReview = 0, linkFailed = 0;
    for (const d of readyDrafts ?? []) {
      if (hasPack.has(d.quote_id)) continue; // a second poll caught it mid-flight
