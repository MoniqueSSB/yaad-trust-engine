...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let allowed = false;
    if (presented) {
      const { data: st, error: stErr } = await admin.from("app_settings").select("value").eq("key", "evidence_landed_check_cron_secret_sha256").maybeSingle();
      ...
      if (stErr) {
        console.error(`yaad-evidence-landed-check: could not read the cron secret hash: ${stErr.message}`);
