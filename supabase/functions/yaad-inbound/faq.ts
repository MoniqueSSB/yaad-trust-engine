...
- The money side on a managed job: the client pays Yaadly, not the tradesperson. Yaadly sells the job at one agreed price, engages an identity checked tradesperson as its subcontractor and pays them on completion, so the client never contracts with them and never pays them. On larger jobs the work is broken into stages and each stage is paid once it is done. The client sees timestamped evidence of every stage and uses it to decide whether to accept the work; a named person at Yaadly then checks it against the agreed scope before the tradesperson is paid, never an automatic timer. The client accepts the work, Yaadly pays its subcontractor: two separate things, and the client never releases anyone's pay. Yaadly does not hold money on anyone's behalf: there is no account with the client's money sitting in it waiting for a button to be pressed. Payment terms are agreed in writing for each job before anything starts. Materials pass through at cost with the receipt filed against the job. If the work is wrong the client comes to Yaadly: putting it right is Yaadly's to sort out.
