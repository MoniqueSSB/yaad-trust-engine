...
- The money side: the client pays Yaadly, not the tradesperson. Yaadly sells the job at one agreed price, engages a vetted tradesperson and pays them directly, so the client never contracts with them. The tradesperson is not paid for a stage until the client approves timestamped evidence from the site, and a named person approves every release, never an automatic timer. Payment terms are agreed in writing for each job before anything starts. Materials pass through at cost with the receipt filed against the job. If the work is wrong the client comes to Yaadly: Yaadly reviews the works and makes sure issues are handled.
