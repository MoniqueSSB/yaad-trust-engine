...
| `yaad-twilio-setup` | true | Creates Yaadly's WhatsApp content templates in Twilio from `content.ts` (the section menu, 15 Sep 2026) and records the ContentSid in `app_settings.twilio_content_sid_phase`, where `yaad-inbound` reads it when the secret is unset. Idempotent, creates content only, sends nothing. Cron secret (env or any cron job's SHA-256 in `app_settings`) or admin session |
...
| `yaad-report-read` | true | The reading half of a priced report: a photographed quote, receipts, message screenshots and site photographs (a PDF arrives as one image per page, rendered in the desk) come back as a verbatim transcription per document and a factual caption per photograph, for the person to correct in the notes box BEFORE the draft is made. Admin session only. It drafts nothing, rates nothing and is not screened, on purpose: it is the record of the source, not the agent's words. Pictures go to `pickVisionProvider("report")`, Mistral EU. Added 25 Sep 2026 |
...
| `yaad-checkout` | true | Card payment, phase 1 (14 Sep 2026, Stripe test keys only). A signed-in client asks to pay one of their own sent, unpaid Yaadly invoices; it checks the invoice is theirs (the `invoices_client_read` email rule), refuses one Stripe already reports paid, and returns a Stripe Checkout address for exactly its total (J$ sent to Stripe in cents, via `_shared/stripe.ts`). Refuses a live key unless `STRIPE_ALLOW_LIVE=yes`. Never marks anything paid |
| `yaad-stripe-webhook` | **false** | Stripe's `checkout.session.completed` / `async_payment_succeeded`. Carries its own authentication: Stripe's HMAC signature over the raw body with `STRIPE_WEBHOOK_SECRET` (`_shared/stripe.ts`); a missing secret is refused 503, a bad signature 403. Writes one `invoice_payments` row per session ('succeeded' if the amount matches the invoice, 'mismatch' if not). **Never writes `invoices.status`**: a named person marks the invoice paid at the desk |
| `yaad-pay` | **false** | The one-click "Pay this invoice by card" link in an emailed client invoice (14 Sep 2026). Carries its own authentication: `?i=<invoice>&t=<token>`, the token an HMAC of the invoice number under the service role key (`_shared/stripe.ts`); wrong or missing token gets the same "not valid" page as an unknown invoice. Checks the invoice is Yaadly's, sent, unpaid, card-payable and not already paid by card, then 303s to a fresh Stripe Checkout page for exactly its total. Serves its own thank-you and cancelled pages. Refuses a live key unless `STRIPE_ALLOW_LIVE=yes`. **Never marks anything paid** |
| `yaad-payout-setup` | true | A worker sets up how Yaadly pays them, on Stripe's own form (14 Sep 2026, Stripe test keys only). A signed-in worker posts `start` or `status`. `start` makes them a Stripe Global Payouts recipient if they are not one (a person in Jamaica, paid in J$ to a local bank; only email and name sent) and returns Stripe's one-time sign-up link, which is never put in a message. `status` reads whether local bank payouts are active. Writes only `worker_profiles.stripe_recipient_*`. Never sees a bank detail, never sends money. Refuses a live key unless `STRIPE_PAYOUTS_ALLOW_LIVE=yes` |
| `yaad-payout-send` | true | A named admin pays a worker through Stripe Global Payouts (16 Sep 2026, `20260916120000`). `quote` reads the invoice, the worker's Stripe recipient, Yaadly's GBP financial account and asks Stripe for the fee, rate and what lands in J$; moves nothing. `send`, only with the desk's second click carrying the quote id and an acknowledgement, creates the OutboundPayment, records it in `stripe_payouts`, then calls `mark_worker_paid` under the person's own session so `paid_by` is them. `refresh` reads a payout's status back. Refuses a live key without `STRIPE_PAYOUTS_ALLOW_LIVE=yes`, refuses a worker without Call-back done, refuses when the GBP balance is short. Never sends on its own. |
| `yaad-wise-recipient` | true | A signed-in worker's bank details, sent straight to Yaadly's Wise Business account as a recipient (14 Sep 2026, `20260914240000`). Checks them against Wise's own rules for JMD to Jamaica, creates the recipient with `WISE_API_TOKEN` (a UK personal token: can create and read recipients, cannot fund a transfer), and records only Wise's id on `worker_profiles`, which clears the worker's call-back. Never stores, logs or traces the details. Unset token means the form says it is not switched on |
