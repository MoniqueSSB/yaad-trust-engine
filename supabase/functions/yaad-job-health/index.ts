...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!allowed && presented) {
      const { data: st, error: stErr } = await admin.from("app_settings").select("value").eq("key", "job_health_cron_secret_sha256").maybeSingle();
      ...
      if (stErr) {
        console.error(`yaad-job-health: could not read the cron secret hash: ${stErr.message}`);
