...
| `preview/` | A clickable, illustrative prototype (no Supabase, nothing saved) used to settle product and pricing decisions before they are built into `docs/`. | **Local only.** This cell said `yaadly.co.uk/preview/` until 19 Sep 2026, on Monique's instruction to fix it. Pages publishes `docs/` and there is no `docs/preview/`, so that URL returns 404 and always has. Read it with `python3 -m http.server 8934 --directory preview`. Publishing it means copying it into `docs/`, which makes every claim in it public. |
...
| `scripts/backup-db.sh` | An off-site copy Monique holds herself. Supabase Pro keeps daily backups for 7 days, but they live inside Supabase and restore the whole database at once; this dump does neither. Point-in-time recovery is a paid add-on and is not switched on. Writes outside any git repository on purpose; refuses to run into a working tree. | Run by hand |
...
- Supabase is on the **Pro plan** since 16 September 2026 (it was free before that, which is why older entries in `DECISIONS.md` and `RUNBOOK.md` talk about free-plan limits). Daily backups, 7 days kept, no pausing for inactivity. Point-in-time recovery, bigger compute and the Team plan are paid extras, not switched on: raise them only if there is a concrete reason, because each one is a spending decision. Leave the spend cap on.
...
- **`--no-verify-jwt` is per function, never a blanket.** Most functions run with `verify_jwt = true` and the platform checks the token before the code runs. Passing the flag turns that off, silently, and the deploy still succeeds. On 30 August 2026 the blanket form of this rule would have stripped platform auth from `yaad-agent`, `yaad-completion`, `yaad-invoice`, `yaad-kickoff`, `yaad-post-job` and `yaad-sketch`. Read the live setting first with `supabase functions list --project-ref <ref>` and preserve what is there. Only the endpoints that carry their own authentication get the flag. **Read it live, do not trust this sentence:** on 3 September 2026 the regression audit found ten functions running without platform auth where this line named four, which is the failure mode the line exists to prevent. As of that check: `yaad-inbound`, `yaad-vetting-review`, `yaad-vetting-upload`, `yaad-enquiry`, `yaad-website-intake`, `yaad-post-job`, `yaad-book-service`, `yaad-portal-code`, `yaad-quote-landed` and `yaad-notify-client`. The six that were added are public endpoints carrying their own throttle, signature or origin check, so the expansion is legitimate; the list going stale is not, because this list is the control. **Re-verified live 4 September 2026: still exactly these ten, no additions and no removals.** **5 September 2026: eleven.** `yaad-message-status` was deployed with the flag and belongs on the list: Twilio posts delivery receipts to it and holds no Supabase session, so the HMAC signature over the URL and the sorted parameters is the only door, the same shape as `yaad-inbound`. Verified by probe rather than by reading the deploy output: an unsigned POST returns 403 "Signature check failed." `yaad-phone-check` and `yaad-vision` were deployed in the same pass **without** the flag and are deliberately not on this list; both return 401 from the platform with no token. `scripts/check-deploy-drift.sh` prints the live list alongside deployed-versus-repo drift, so checking it is one command rather than a careful manual comparison nobody repeats. **6 September 2026: twelve.** `yaad-job-photo` was deployed with the flag and belongs on the list: it takes photographs attached on the job form, where the person filling it in has no account by deliberate decision, so the pair (job id, portal code) is the only door and it opens only while the job is unclaimed. Verified live rather than assumed: `supabase functions list` shows exactly these twelve with `verify_jwt:false`, and a probe with a wrong code returns 403 "That job code is not valid." **14 September 2026: thirteen.** `yaad-stripe-webhook` was deployed with the flag and belongs on the list: Stripe posts card payment events to it and holds no Supabase session, so Stripe's HMAC signature over the raw body, checked against `STRIPE_WEBHOOK_SECRET` in `_shared/stripe.ts`, is the only door. It records a payment and never marks an invoice paid. Verified by probe, not by reading the deploy output: an unsigned POST and a POST with a forged `Stripe-Signature` both return 403, and `supabase functions list` shows exactly these thirteen with `verify_jwt:false`. `yaad-checkout`, deployed in the same pass, keeps the platform check and is deliberately not on this list; an unsigned call returns 401. **14 September 2026, later the same day: fourteen.** `yaad-pay` was deployed with the flag and belongs on the list: it is the one-click "Pay this invoice by card" link in an emailed invoice, tapped by a client who holds no session, so the link's own token (an HMAC of the invoice number under the service role key, `_shared/stripe.ts`) is the only door. It makes a Stripe payment page for exactly one sent, unpaid invoice and never marks anything paid. Verified by probe, not by reading the deploy output: a tampered token and a missing token both return 404 with the function's own "This payment link is not valid" page, and `supabase functions list` shows exactly these fourteen with `verify_jwt:false`. `yaad-checkout` and `yaad-invoice`, redeployed in the same pass, keep the platform check; unsigned calls return 401. (`yaad-whatsapp-webhook` was on this list; it spoke to Meta's Cloud API directly, never received real traffic, and was deleted 1 Sep 2026, see DECISIONS.md. Real WhatsApp intake runs through `yaad-inbound`, over Twilio.)
- **Name a new migration so it SORTS LAST: `<14-digit timestamp>_<lower_snake_name>.sql`.** The old scheme was a date and a single letter, `20260904a`, `20260904b`. Two sessions cannot see each other's branches, so both take the next free letter and both are right; on 5 September 2026 that collided four times in one day. It never shows up as a git conflict, because the filenames differ, so both sets merge silently into a directory whose whole purpose is order. `supabase migration new <name>` produces the right shape. The 182 existing letter-named files are applied history and stay as they are. Enforced by `scripts/check-migration-order.mjs` in CI, which checks that a newly added file sorts after every migration already on the base branch, not merely that it looks like a timestamp: a stamp added on a day that already has letter files sorts before them, because `0` sorts before `a`.
