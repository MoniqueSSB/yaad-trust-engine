...
- **jobChat**: in-portal chat, relayed to WhatsApp both ways, fully automated (see `specs/PORTALS-BUILD-SPEC.md` §5.4 for which function actually carries each direction as of 1 Sep 2026; `yaad-whatsapp-webhook` no longer exists). The scrub regex set (email, number, spelled-out number, off-platform app, off-platform payment, handle) runs on insert AND render. A blocked message shows what was removed and why; never silently drop.
