...
Header, about in their own words, verification checks in order with the police-check consequence stated plainly where absent ("Cannot be matched to jobs over £500 or work inside an occupied home"), portfolio by evidence weight, and BOTH review panes: reviews of the worker and reviews of the client. The client pane is not optional; the mirror rule is real or it is marketing.
...
DONE: open_jobs and client_summary views; structured-row columns and job_photos migrated; board with both toggles, count line, eight-row card, photo expand, lock note, live fee split wired to the real job_quotes insert; worker directory tab; render-verified live with a temporary test row, then deleted.
KNOWN GAP: the view's address scrub is line-anchored (strips "Address:" lines) and does not catch a free-text address mid-sentence; measured, on the list.
TO BUILD: worker profile screen with both review panes, the reviews system and trigger, trades screen, Ask a Yaad, interest counts on cards.
